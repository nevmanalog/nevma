...
interface Props {
  ...
  // Live-preview hooks, both optional so nothing breaks if a caller doesn't
  // wire them up. onPreviewBegin fires once as a stroke starts; onPreviewDelta
  // fires for each newly-accepted sample as a tiny 2-point SCREEN-space
  // segment [prevX, prevY, newX, newY] — not the whole stroke so far, which
  // is what keeps it cheap no matter how long the stroke gets.
  onPreviewBegin?: () => void
  onPreviewDelta?: (segment: number[]) => void
}
 ...
 */
export function PhysicalToolInput({ onStroke, cursor: cursorSpec, navGesture, onPreviewBegin, onPreviewDelta }: Props) {
  ...
  const move = (e: Konva.KonvaEventObject<MouseEvent | TouchEvent>) => {
    ...
    const prevX = sp[sp.length - 2]
    const prevY = sp[sp.length - 1]
    screenRef.current = [...sp, s[0], s[1]]
    setPreview((prev) => [...prev, world.x, world.y])
    onPreviewDelta?.([prevX, prevY, s[0], s[1]])
  }
